...
impl Pipeline {
    pub fn new(device: &wgpu::Device, format: wgpu::TextureFormat, backend: wgpu::Backend) -> Self {
        ...
        let constant_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("enative_wgpu::image constants layout"),
            entries: &[
        ...
        let texture_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("enative_wgpu::image texture atlas layout"),
        ...
        let layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("enative_wgpu::image pipeline layout"),
            bind_group_layouts: &[&constant_layout, &texture_layout],
            immediate_size: 0,
        });

        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("enative_wgpu image shader"),
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("enative_wgpu::image pipeline"),
            layout: Some(&layout),
...
impl Layer {
    fn new(
        ...
        let uniforms = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("enative_wgpu::image uniforms buffer"),
        ...
        let instances = Buffer::new(
            device,
            "enative_wgpu::image instance buffer",
            Instance::INITIAL,
            wgpu::BufferUsages::VERTEX | wgpu::BufferUsages::COPY_DST,
        );

        let nearest_layout = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("enative_wgpu::image constants bind group"),
